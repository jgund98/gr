...
export default function Marquee() {
  const strip = (hidden: boolean) => (
    <img
      ...
      className="h-[76px] w-auto max-w-none shrink-0 md:h-24"
      loading="eager"
      decoding="sync"
    />
  );
  return (
    <div className="relative overflow-hidden bg-green py-2.5 md:py-3">
      <div className="animate-marquee flex w-max [animation-duration:26s] md:[animation-duration:34s]">
        {strip(false)}
        {strip(true)}
      </div>
    </div>
